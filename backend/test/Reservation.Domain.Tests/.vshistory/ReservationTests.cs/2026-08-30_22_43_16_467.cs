...
namespace Reservation.Domain.Tests {
    public class ReservationTests {
        ...
        [Fact]
        public void 已確認開席但超過30分鐘內席則視為NoShow() {
            var r = 建立有效訂位(3);
            r.Confirm(TableNo.Of("A5"),Now);
            //if no show for 30 minitues,set to now show
            var nowToTestNowShow =r.Slot.StartAt.AddMinutes(25);
            Assert.Throws<DomainException>(() => r.NoShow(nowToTestNowShow));
        }
        [Theory]
        [InlineData("09A1234578")]
        [InlineData("091234567899")]
        public void PhoneNumberFormatTest(string phoneNum) {
            var act = () => PhoneNumber.Of(phoneNum);
            Assert.Throws<DomainException>(act);
        }
