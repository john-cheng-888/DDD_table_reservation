...
namespace Reservation.Infrastructure.Persistence {
    ...
    public sealed class EfReservationRepository(AppDbContext db) : IReservationRepository {
        public async Task AddAsync(ReservationAggregate reservation, CancellationToken ct) {
            await db.Reservations.AddAsync(reservation,ct); 
        }

        public async Task<ReservationAggregate?> FindAsync(ReservationId id, CancellationToken ct) {
        ...
        public async Task<IReadOnlyList<ReservationAggregate>> FindByDateAsync(DateOnly date, CancellationToken ct) {
