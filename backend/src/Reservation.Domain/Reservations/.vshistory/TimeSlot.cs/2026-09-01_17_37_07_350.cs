...
namespace Reservation.Domain.Reservations {
    public sealed class TimeSlot :ValueObject{
        ...
        public bool Overlaps(TimeSlot other) => !IsSeparateFrom(other);


        private bool IsSeparateFrom(TimeSlot other) =>
            EndAt <= other.StartAt || other.EndAt <= StartAt;
        ...
        /// </summary>
        /// <returns></returns>
        public TimeSlot WithTurnoverBuffer(TimeSpan buffer) => Of(StartAt, Duration + buffer);
