...
//允許多桌併桌
        ...
        public static Reservation Place(Guest guest, TimeSlot slot, PartySize size, DateTimeOffset now,string? note="") {
            if (slot.StartAt < now) {
            ...
            r.Raise(new ReservationPlaced(r.Id, slot.StartAt, size.Value,note, now));
            return r;
        }
        public void setNote(string note) {
            if (note.Trim().Length > 200) {
                throw new ArgumentException("備註長度不可超過200個字");
            }
            Note = note;
        }
